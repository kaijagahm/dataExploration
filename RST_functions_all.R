...
# Downloaded by Kaija Gahm on 2022-10-05 from https://github.com/raorben/RST/blob/master/RST_functions_all.R.
...
chooseDynScale<-function(scales,radius,bandIDs){
  ...
  for (i in 1:length(bandIDs)) {
    ...
    TransRes<-c(Row[,1],Row[,(length(scales)-length(radius)+1):length(scales)]) # XXX transit points are residuals with a value of 0.
