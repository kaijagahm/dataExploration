...
fssf <- feedingStations %>%
  sf::st_as_sf(coords = c("ITM_E", "ITM_N"), remove = F) %>%
  sf::st_set_crs(6991) %>%
  sf::st_transform("WGS84")

ggplot(bg)+
  geom_sf(data = bg)+
  geom_sf(data = fssf, color = "blue", alpha = 0.5, size = 4)+
  theme_minimal()+
  ylim(29.4, 31.6)
```
...
plotsElev <- map2(eventsDat, eventsList, ~{
  .x %>%
    mutate("Height above \nground (m)" = height_above_ground) %>%
    ggplot()+
    geom_line(aes(x = timestamp, y = kmFromCarcass, group = trackId), alpha = 0.3)+
    geom_point(aes(x = timestamp, y = kmFromCarcass, col = `Height above \nground (m)`))+
    scale_color_viridis_c()+
    geom_vline(aes(xintercept = .y$datetime[1]))+
    theme_minimal()+
    ylim(0, 5)+
...
```{r}
firstsLong <- firsts %>%
  pivot_longer(cols = c(detection, visit), names_to = "type", values_to = "hours")
  
firstsLong %>%
  ggplot(aes(x = hours, col = type, fill = type))+
  geom_density(alpha = 0.5)+
  geom_vline(aes(xintercept = 24), alpha = 0.7)+
  geom_vline(aes(xintercept = 48), alpha = 0.5)+
  geom_vline(aes(xintercept = 72), alpha = 0.3)+
  geom_vline(aes(xintercept = 96), alpha = 0.1)+
  scale_color_manual(name = "", values = c("lightblue", "#FCA18E"))+
  scale_fill_manual(name = "", values = c("lightblue", "#FCA18E"))+
  theme_classic()+
  ylab("Frequency")+
  xlab("Time (hours)")+
  theme(text = element_text(size = 20))
...
firstVisits %>%
  ...
  theme(legend.position = "none")+
  ylab("# unique individuals")+
  xlab("Time since first visit (hours)")+
  theme(text = element_text(size = 18))
```
...
# What about *relative* detection order and rank order?
dfCombined %>% # XXX COME BACK TO THIS--NEED TO FILTER OUT THE DETECTIONS THAT ARE ALSO VISITS
  group_by(eventID) %>%
  mutate(relative_order_detection = order_detection/max(order_detection, na.rm = T),
         relative_order_visit = order_visit/max(order_visit, na.rm = T)) %>%
  ggplot(aes(x = relative_order_detection, y = relative_order_visit, col = factor(eventID)))+
  geom_point(alpha = 0.5, size = 2)+
  geom_smooth(method = "lm", se = F, size = 0.5)+
  theme_minimal()+
  ylab("Relative rank order (visits)")+
  xlab("Relative rank order (detections)")+
  theme(legend.position = "none")+
  theme(text = element_text(size = 20))
```
How consistent is individual normalized rank order across feeding events?
...
byIndiv <- dfCombined %>%
  group_by(trackId) %>%
  summarize(mn_order_detection = mean(order_detection, na.rm = T),
            sd_order_detection = sd(order_detection, na.rm = T),
            mn_order_visit = mean(order_visit, na.rm = T),
            sd_order_visit = sd(order_visit, na.rm = T),
            mn_relative_order_detection = mean(relative_order_detection, na.rm = T),
            sd_relative_order_detection = sd(relative_order_detection, na.rm = T),
            mn_relative_order_visit = mean(relative_order_visit, na.rm = T),
            sd_relative_order_visit = sd(relative_order_visit, na.rm = T))

head(byIndiv)

# boxplot
ord_det <- dfCombined %>% group_by(trackId) %>% summarize(mn_det = mean(relative_order_detection, na.rm = T)) %>% arrange(mn_det) %>% pull(trackId)
ord_vis <- dfCombined %>% group_by(trackId) %>% summarize(mn_vis = mean(relative_order_visit, na.rm = T)) %>% arrange(mn_vis) %>% pull(trackId)

dfCombined %>%
  mutate(trackId = factor(trackId, levels = ord_det)) %>%
  ggplot(aes(x = trackId, y = relative_order_detection))+
  geom_boxplot()+
  geom_jitter(position=position_jitter(0), alpha = 0.5)+
  theme(panel.grid = element_blank())+
  ylab("Relative rank order (detection)")+
  theme(axis.text.x = element_blank(),
        axis.ticks.x = element_blank())+
  xlab("Individual")+
  ggtitle("Relative rank order (detections)")

dfCombined %>%
  mutate(trackId = factor(trackId, levels = ord_vis)) %>%
  ggplot(aes(x = trackId, y = relative_order_visit))+
  geom_boxplot()+
  geom_jitter(position=position_jitter(0), alpha = 0.5)+
  theme(panel.grid = element_blank())+
  ylab("Relative rank order (visit)")+
  theme(axis.text.x = element_blank(),
        axis.ticks.x = element_blank())+
  xlab("Individual")+
  ggtitle("Relative rank order (visits)")
```
...
lccProportionsFunction <- function(whichEvent, days){
  # create a named vector of daysBack numbers to use
  daysBack <- seq(from = 1, to = days, by = floor(days/10))
  names(daysBack) <- as.character(daysBack)
  
  # get the date and time of the feeding event
  ...
  # Get data prior to the event, as far back as `days`. 
  priorData <- datMasked %>%
  ...
  # Get initial visit and initial detection for this feeding event.
  idd <- firstDetections %>%
    filter(eventID == whichEvent) %>%
    pull(timestamp) %>%
    first()
  ...
  # Create feeding and flying edges for this feeding event
  ...
  outList <- list("propData" = out,
                  ...
                  "feedingEdges" = feedingEdges,
                  "flightEdges" = flightEdges,
                  "feedingEdges_nested" = feedingEdges_nested,
                  "flightEdges_nested" = flightEdges_nested,
                  "feedingEdges_graphs" = feedingEdges_graphs,
                  "flightEdges_graphs" = flightEdges_graphs,
                  "nDays_feeding" = nDays_feeding,
                  "nDays_flight" = nDays_flight
                  )
  cat("Retrieving and computing data for feeding event #", whichEvent, "\n")
  return(outList)
}
```

Actually run the above function for all the feeding events, to collect data. (This part takes a while.)

```{r}
data_allEvents <- map(1:length(eventsList), ~{lccProportionsFunction(whichEvent = .x, days = 60)})
save(data_allEvents, file = "data/data_allEvents.Rda")
load("data/data_allEvents.Rda")

# Get just the proportion data
propData <- map_dfr(data_allEvents, "propData") %>%
  pivot_longer(cols = c(propIncluded_flight, propIncluded_feeding), names_to = "situation", 
               values_to = "propIncluded") %>%
  mutate(situation = stringr::str_remove(situation, "propIncluded_"))
```

Calculate the number of days needed for 90%:
```{r}
# Important: this analysis will only include feeding events that DO eventually reach 90%!
# For each feeding event, retrieve the first time that propIncluded goes above 90% for each of feeding and flight
ninetyPerCent <- propData %>%
  filter(propIncluded >= .9) %>%
  group_by(eventID, situation) %>%
  arrange(daysBack, .by_group = T) %>%
  slice(1)
```
...
ninetyPerCent %>%
  ggplot(aes(x = situation, y = daysBack, fill = situation))+
  geom_boxplot()+
  geom_jitter(width = 0.1, size = 3, alpha = 0.7)+
  theme_minimal()+
  ylab("# days for 90% in network")+
  xlab("")+
  theme(legend.position = "none")+
  theme(text = element_text(size = 20))+
  geom_text(data = nData, aes(y = 0, label = paste("n =", n, "feeding events")))

```


Make an accumulation plot:
```{r}
propData %>%
  filter(!is.na(propIncluded)) %>%
  ggplot(aes(x = daysBack, y = propIncluded, col = factor(eventID)))+
  geom_point()+
  geom_line()+
  facet_wrap(~situation)+
  theme_minimal()+
  theme(legend.position = "none")+
  ylab("Prop. indivs. included")+
  xlab("Network time interval (days)")+
  theme(text = element_text(size = 20))
```
