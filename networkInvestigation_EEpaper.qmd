...
if(includeAllVertices){
  uniqueIndivs <- unique(dataset$Nili_id)
}
...
durations_flight %>%
  ...
  arrange(desc(nTimegroups))

# Looking at the longest one, between armstrong and kat
check <- flight %>%
  filter(timegroup >= 322 & timegroup <= 347 & ID1 == "armstrong" & ID2 == "kat")

# Extract the timestamps and go back to the data
mn <- min(check$minTimestamp)
mx <- max(check$maxTimestamp)

checkData1 <- subsetted %>% filter(Nili_id %in% c("armstrong", "kat"), timestamp >= mn, timestamp <= mx) %>%
  sf::st_as_sf(coords = c("location_long", "location_lat")) %>%
  sf::st_set_crs("WGS84")

mapview::mapview(checkData1, zcol = "Nili_id") # huh, so they seem to be flying together for a very long way, never more than a couple hundred meters apart. This strikes me as a bit suspicious. Do they actually fly that tightly together? Or was this a mis-recording of a single bird?
...
check <- flight %>%
  filter(timegroup >= 1007 & timegroup <= 1014 & ID1 == "nevada" & ID2 == "phili")

# Extract the timestamps and go back to the data
mn <- min(check$minTimestamp)
mx <- max(check$maxTimestamp)

checkData2 <- subsetted %>% filter(Nili_id %in% c("nevada", "phili"), timestamp >= mn, timestamp <= mx) %>%
  sf::st_as_sf(coords = c("location_long", "location_lat")) %>%
  sf::st_set_crs("WGS84")

mapview::mapview(checkData2, zcol = "Nili_id") # looks pretty similar to the last one. But two different individuals. Maybe they just do this periodically!
```
...
When does the SRI correction happen? Does it happen for each ten-minute interval, or overall? Do this first. When does Nitika implement it in her code? \[I just checked, and it looks like she does it at the end.\]
...
nrow(bigGroup) # 29 edges during this timegroup
