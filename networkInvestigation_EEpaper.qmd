...
View(example) # as we can see, we have interactions in consecutive time slices. Those are treated as separate edges in the network.
``` 
...
```{r}
# Here: test out what would happen if we condensed interactions and added duration information.
```
...
uniqueIndivs <- unique(subsetted$trackId)
allPairs <- expand.grid(uniqueIndivs, uniqueIndivs) %>%
  mutate(across(everything(), as.character)) %>% # remove duplicates and self edges
  filter(Var1 > Var2)
indivTimegroupOccurrences <- flight %>%
  select(timegroup, ID1, ID2) %>%
  pivot_longer(cols = -timegroup, values_to = "trackId") %>%
  select(-name) %>%
  distinct() %>%
  arrange(trackId, timegroup)

# how many timegroups each pair of individuals *co-occurred* in
sameTimegroup <- crossprod(table(indivTimegroupOccurrences))
sameTimegroup[upper.tri(sameTimegroup)] <- 0
sameTimegroupLong <- as.data.frame.table(sameTimegroup) %>%
...
# how many timegroups each pair of individuals *interacted* in
interactTimegroup <- flight %>%
  select(timegroup, ID1, ID2) %>%
  group_by(ID1, ID2) %>%
  summarize("Freq" = n()) %>%
  ungroup() %>%
  mutate(type = "interact")

sri <- bind_rows(sameTimegroupLong, interactTimegroup) %>%
  pivot_wider(id_cols = c("ID1", "ID2"), names_from = "type", values_from = "Freq")
```
