...
checkData <- subsetted %>% filter(trackId %in% c("A00w", "J12w"), timestamp >= mn, timestamp <= mx) %>%
  sf::st_as_sf(coords = c("location_long", "location_lat")) %>%
  sf::st_set_crs("WGS84")

mapview::mapview(checkData, zcol = "trackId") # huh, so they seem to be flying together for a very long way, never more than a couple hundred meters apart. This strikes me as a bit suspicious. Do they actually fly that tightly together? Or was this a mis-recording of a single bird?
...
checkData <- subsetted %>% filter(trackId %in% c("T25b", "T76w"), timestamp >= mn, timestamp <= mx) %>%
  sf::st_as_sf(coords = c("location_long", "location_lat")) %>%
  sf::st_set_crs("WGS84")

mapview::mapview(checkData, zcol = "trackId") # looks pretty similar to the last one. But two different individuals. Maybe they just do this periodically!
```


Re-examine the number of interactions per month after incorporating durations.
```{r}
...
subset_m2 <- m2[indivs_m1, indivs_m1]
dim(subset_m2)
dim(m1)
...
# Multiple interacting individuals are just given their own edges
```
