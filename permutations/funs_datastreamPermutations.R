...
p_randomDays <- function(dataset, idCol = "trackId", dateCol = "dateOnly", timeCol = "timeOnly"){
  permuted <- dataset %>%
    group_by(.data[[idCol]]) %>%
    group_split(.keep = T) %>%
    map_dfr(~{
...
p_shift <- function(dataset, shiftMax = 10, idCol = "trackId", dateCol = "dateOnly", timeCol = "timeOnly"){
  out <- dataset %>%
    group_by(.data[[idCol]]) %>%
    group_split(.keep = T) %>%
    map_dfr(~{
...
p_conveyor <- function(dataset, shiftMax, mode = "global", idCol = "trackId", dateCol = "dateOnly", timeCol = "timeOnly"){
