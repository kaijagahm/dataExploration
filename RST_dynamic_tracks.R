...
## Modified again by Kaija Gahm, 2022-10-05
#########################################################

# --------------------- LOADING FUNCTIONS AND LIBRARIES --------------------- #
# load C function, secondary functions, plotting functions
source("RST_functions_all.R")
dyn.load("RST_residenceRadii.so") # XXX I had to "allow" this in Mac system preferences.
...
origData <- read.csv("data/GHAL_23059.csv") # XXX basic dataset with datetime, lat, long, and bird ID. Note that this dataset contains only one individual. Would have to wrap it into a list for multiple individuals.
...
dataset <- data.frame("band" = origData$band,
                      "lat" = origData$lat,
                      "lon" = origData$lon,
                      "datetime" = strptime(origData$Datetime_GMT, format = "%d/%m/%Y %H:%M:%S")) #XXX can use sf for this instead
...
lambert <- mapproject(dataset$lon, dataset$lat, projection = "lambert", parameters = c(mean(dataset$lon), mean(dataset$lat))) # XXX can use sf for this instead.
scale <- haversineDist(min(dataset$lon), min(dataset$lat), max(dataset$lon), max(dataset$lat)) / projectDist(min(lambert$x), min(lambert$y), max(lambert$x), max(lambert$y), 1) # XXX I think this will be obsolete if I just use sf with an appropriate CRS? Seems like they are doing a manual transformation.
...
saveRDS(MoveResid,file = ("data/GHAL_23059_allscales.rda"))
#MoveResid<-readRDS("data/GHAL_23059_allscales.rda")
...
pdf("data/GHAL_23059_MultiScale.pdf")
...
saveRDS(MoveResid_DynScale,file = paste0("data/GHAL_DynScale.rda"))
#MoveResid_DynScale<-readRDS(paste0(dir,"data/GHAL_DynScale.rda"))
...
for (i in 1:length(bandIDs)) {
  ...
  quartz.save(file=(paste0('data/GHAL_',radius,'.png')), type = "png", device = dev.cur(), dpi = 100)
  #savePlot(file=(paste0('data/GHAL_',radius,'.png')), type = "png", device = dev.cur()) #for Windows users
  dev.off()
}
