...
datAnnotCleaned <- sf::st_drop_geometry(datAnnotCleaned)
...
datAnnotSimp <- datAnnot %>%
  filter(status == "valid") %>%
  dplyr::select(Nili_id, timestamp, dateOnly, acceleration_raw_x, acceleration_raw_y, acceleration_raw_z, ground_speed, heading, location_lat, location_long, status)

# get any "ghost data", aka "valid" data occurring after the date of death
ghostData <- lapply(orniDeathsList, function(x){
  ghostData <- datAnnotSimp %>%
    filter(Nili_id == x$Nili_id[1]) %>%
    filter(dateOnly > x$date_death[1])
})
indivs <- lapply(ghostData, function(x){x$Nili_id[1]}) %>% unlist()
```
...
```{r}
whoDied <- lapply(orniDeathsList, function(x){x$Nili_id[1]})
...
nIndivsDaily <- suppressMessages(pmap(.l = list("dat" = orniDeathsDat, "date" = deathDates, "who" = whoDied), .f = function(dat, date, who){
  out <- dat %>% 
    dplyr::select(dateOnly, Nili_id) %>% 
    distinct() %>% 
    mutate(deathDate = date) %>%
    group_by(dateOnly, deathDate) %>% 
    summarize(nIndivs = length(unique(Nili_id))) %>%
    mutate(whoDied = who)
  return(out)
})) %>%
...
datAnnotCleanedSimp <- datAnnotCleaned %>%
  filter(status == "valid") %>%
  dplyr::select(Nili_id, timestamp, dateOnly, acceleration_raw_x, acceleration_raw_y, acceleration_raw_z, ground_speed, heading, location_lat, location_long, status, gps_time_to_fix, gps_satellite_count)
...
dailyFlightGraphs <- map(dailyFlightEdges, ~vultureUtils::makeGraph(.x, mode = "edgelist"))
```

Create flight networks for the days before and after the death date
```{r}
df <- orniDeathsDat[[2]] %>%
  sf::st_as_sf(., coords = c("location_long", "location_lat"), remove = F) %>%
  sf::st_set_crs(., "WGS84")
before <- df %>% filter(dateOnly < deathDates[[2]])
after <- df %>% filter(dateOnly > deathDates[[2]])
elBefore <- vultureUtils::getFlightEdges(dataset = before, roostPolygons = roostPolygons, roostBuffer = 50, consecThreshold = 2, distThreshold = 250)
elAfter <- vultureUtils::getFlightEdges(dataset = after, roostPolygons = roostPolygons, roostBuffer = 50, consecThreshold = 2, distThreshold = 250)
```
