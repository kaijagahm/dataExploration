...
```{r}
datAnnotCleaned <- load("data/datAnnotCleaned.Rda")
...
datAnnotSimp <- datAnnot %>%
  filter(status == "valid") %>%
  dplyr::select(trackId, timestamp, dateOnly, acceleration_raw_x, acceleration_raw_y, acceleration_raw_z, ground_speed, heading, location_lat, location_long, status)
...
dailyFlightGraphs <- vultureUtils::makeGraphsList(dataList = dailyFlightEdges)$graphs
```
...
gBefore <- vultureUtils::makeGraphs(elBefore, interval = "5 days")$graphs
gAfter <- vultureUtils::makeGraphs(elAfter, interval = "5 days")$graphs

plot(gBefore[[length(gBefore)]])
plot(gAfter[[1]])
```
