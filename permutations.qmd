...
#save(roostNetworks_polygon, file = "data/roostNetworks_polygon.Rda")
load("data/roostNetworks_polygon.Rda")
```

## Create networks
### Flight
```{r}
flightSRI <- map(flightNetworks, "sri")
test <- flightSRI[[6]]
g <- vultureUtils::makeGraph(mode = "sri", data = test, weighted = F, id1Col = "ID1", id2Col = "ID2")
plot(g, mode = "undirected", vertex.size = 10, edge.width = 50*as.numeric(E(g)$weight))
```

### Feeding
```{r}
feedingSRI <- map(feedingNetworks, "sri")
```

### Roost (distance and polygon)
```{r}
roostSRI_distance <- map(roostNetworks_distance, "sri")
roostSRI_polygon <- map(roostNetworks_polygon, "sri")
```
