...
Make an animation for a single carcass over a few days. I'm going to use feeding event 36, since it seems relatively simple.
...
p <- ggplot()+
  ...
  geom_path(data = df_all, 
            aes(x = long, y = lat, group = trackId, color = dist), 
            alpha = 0.3, size = 0.5)+
...
frames <- length(unique(df_all$hour))
gganimate::animate(anim, nframes = frames, fps = 10)
