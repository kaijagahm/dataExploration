...
maxTime <- 96
thisEvent <- datMasked %>%
  filter(timestamp > event$datetime[9] - lubridate::as.duration("2 hours") & timestamp < event$datetime[9] + lubridate::as.duration(maxTimeWords))
...
```{r results='hide',fig.keep='all', out.width = "50%"}
...
plots <- map2(.x = eventsDat, .y = eventsList, .f = function(.x, .y){
  ...
  p <- .x %>%
    ...
    geom_vline(data = .y, aes(xintercept = .y$datetime[1]))+
    theme_minimal()+
    ylim(0, 20)+
    theme(legend.position = "none")
})
