...
eventsDat <- map2(eventsDat, eventsList, ~.x %>%
                    mutate(distanceFromCarcass_km = as.numeric(sf::st_distance(., .y))/1000))

# download elevation rasters for points within 3km of each feeding event
eventsDat3km <- map(eventsDat, ~.x %>% filter(distanceFromCarcass < 3))
...
plots <- map2(eventsDat, eventsList, ~{
  p <- .x %>%
    ggplot()+
...
suppressWarnings(walk(plots, print))
```
...
allCarcasses <- data.table::rbindlist(eventsDat, idcol = "eventID") %>%
  as.data.frame() %>%
  dplyr::select(eventID, ground_speed, height_above_msl, location_lat, location_long, timestamp, dateOnly, trackId, geometry, hoursSinceCarcassPlaced, distanceFromCarcass, groundElev, height_above_ground, detection, visit)
```
