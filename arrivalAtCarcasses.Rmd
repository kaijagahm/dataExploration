...
### Detections
Now we can do the same thing for detections (i.e. not only visits)
```{r}
# Get just an ordered list of who detected the carcass first in the first 96 hours since the carcass was deposited
detectionOrders <- allCarcasses %>%
  ...
  mutate(hoursSinceCarcassPlaced = timeSinceCarcassPlaced/60) %>%
  filter(hoursSinceCarcassPlaced <= 96) %>%
  dplyr::select(eventID, trackId, timestamp, hoursSinceCarcassPlaced, distanceFromCarcass, detection, detection, hour) %>%
  distinct() %>%
...
Generally, there is a relationship between detection order and visit order. This makes sense, since visits are a subset of detections.
