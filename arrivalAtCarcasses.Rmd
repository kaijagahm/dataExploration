...
# Make the map
ggmap(map)+
  geom_point(data = test, aes(x = location_long.1, y = location_lat.1, col = timestamp), size = 0.2)+
  geom_path(data = test, aes(x = location_long.1, y = location_lat.1, col = timestamp), size = 0.02)+
  scale_color_viridis_c()+
  ggtitle(paste0("T61w from ", min(test$dateOnly), " to ", max(test$dateOnly)))+
...
maxTimeWords <- "72 hours"
maxTime <- 72
nToTest <- 5
thisEvent <- datMasked %>%
  filter(timestamp > feedEvents$datetime[nToTest] - lubridate::as.duration("2 hours") & timestamp < feedEvents$datetime[nToTest] + lubridate::as.duration(maxTimeWords))
# I have not yet restricted this spatially.

bbox <- bboxFromRad(lon = feedEvents$long[nToTest], lat = feedEvents$lat[nToTest], kmDist = 3)
...
ggmap(map)+
  ...
  geom_point(data = event, aes(x = long, y = lat), col = "black", size = 3, pch = 17)+
...
suppressWarnings(walk(plots48, print))
```
...
suppressWarnings(walk(plots24, print))
```
Hourly averages:
...
suppressWarnings(walk(plots48_hourly, print))
```
...
allCarcasses <- data.table::rbindlist(eventsDat, idcol = "eventID") %>%
  as.data.frame() %>%
  dplyr::select(eventID, ground_speed, height_above_msl, location_lat, location_long, timestamp, dateOnly, trackId, geometry, hoursSinceCarcassPlaced, kmFromCarcass, groundElev, height_above_ground, detection, visit)
```
...
df <- firstDetections %>%
  ...
  pivot_wider(id_cols = eventID, values_from = hoursSinceCarcassPlaced, names_from = type) %>%
...
df %>%
  ggplot(aes(x = hoursSinceCarcassPlaced, y = nFirstVisits, 
             col = as.factor(eventID)))+       geom_line()+
  theme_classic()+
  theme(legend.position = "none")+
...
df %>%
  ...
  ggplot(aes(x = as.numeric(timeSinceFirstVisit)/3600, y = nFirstVisits, col = as.factor(eventID)))+       
  geom_line()+
  theme_classic()+
  theme(legend.position = "none")+
  ylab("Number of unique vultures that have visited")+
...
allCarcasses <- allCarcasses %>%
  filter(hoursSinceCarcassPlaced > 0) %>%
  mutate(hour = lubridate::floor_date(timestamp, unit = "hour")) %>%
  mutate(under1 = kmFromCarcass < 1,
         under5 = kmFromCarcass < 5,
         under10 = kmFromCarcass < 10,
         under50 = kmFromCarcass < 50,
         under100 = kmFromCarcass < 100,
         over100 = kmFromCarcass > 100)
...
visitsPerHour <- allCarcasses %>%
  filter(hoursSinceCarcassPlaced < maxTime) %>%
  dplyr::select(eventID, hoursSinceCarcassPlaced, trackId, visit) %>%
  group_by(eventID, trackId) %>%
  arrange(hoursSinceCarcassPlaced, .by_group = TRUE) %>%
  ...
  ungroup() %>%
  mutate(hours = floor(hoursSinceCarcassPlaced)) %>%
...
visitOrders <- allCarcasses %>%
  ...
  dplyr::select(eventID, trackId, timestamp, hoursSinceCarcassPlaced, kmFromCarcass, visit, visit, hour) %>%
  distinct() %>%
  filter(visit == TRUE) %>%
  group_by(eventID) %>%
  arrange(timestamp, .by_group = T) %>%
...
detectionOrders <- allCarcasses %>%
  filter(hoursSinceCarcassPlaced <= 96) %>%
  dplyr::select(eventID, trackId, timestamp, hoursSinceCarcassPlaced, kmFromCarcass, visit, detection, hour) %>%
  distinct() %>%
