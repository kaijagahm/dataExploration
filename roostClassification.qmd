...
plots_numbers <- map(names(testDF_withClusters)[grepl("k_", names(testDF_withClusters))], ~{
  ...
  geom_sf(data = croppedPolygons, fill = NA)+
  theme_classic()+
...
I am feeling very frustrated! On first glance, this seems better than hierarchical or k-means, but overlaying the roost polygons shows us that there are still errors in both directions--polygons that include multiple colors, and multiple polygons overlaid on groups of points of the same color. Of course, this is all complicated by the fact that we don't have nearly enough colors to distinguish all the clusters, so some of these may be different even when they seem the same.

We also have a *ton* of outliers. I don't know if this is inherently a bad thing. One possibility is that we could run DBSCAN first, and then take the outliers and use a different clustering method to assign them to more diffuse groups, which would mean something different from what the DBSCAN groups mean. I haven't tried this yet.

I would really like to be able to explore this more, so I'm going to try using leaflet to visualize the points.
